...
angular.module('acComponents.directives')
    .directive('acForecastMini', ["$state", "$stateParams", "AC_API_ROOT_URL", function ($state, $stateParams, AC_API_ROOT_URL) {
        return {
    ...
    .directive('acForecastMiniAvalx', ["AC_API_ROOT_URL", function (AC_API_ROOT_URL) {
        return {
            restrict: 'E',
    ...
    .directive('acSpringConditionsText', function () {
        return { 
            ...
            template: '<p>' +
                ...
                'crusts fromed overnight and rising air temperatures during the ' +
...
$templateCache.put("forecast-mini.html","<div class=\"panel\"><div class=\"row\"><div ng-click=\"closeDrawer()\" style=\"margin-right: 35px;\" class=\"pull-right close-drawer\"><i class=\"fa fa-close\"></i></div></div><div ng-show=\"forecast.externalUrl\"><ac-forecast-mini-external forecast=\'forecast\' /></div><div ng-show=\"forecast.parksUrl\"><ac-forecast-mini-parks forecast=\'forecast\' /></div><div ng-hide=\"forecast.externalUrl || forecast.parksUrl\"><ac-forecast-mini-avalx forecast=\'forecast\' danger-rating=\'dangerRating\' disclaimer=\'disclaimer\' sponsor=\'sponsor\' /></div></div>");
$templateCache.put("hot-zone-mini.html","<div class=\"panel\"><div class=\"panel-body ac-forecast-mini-body\"><div class=\"row\"><div ng-click=\"closeDrawer()\" class=\"pull-right close-drawer\"><i class=\"fa fa-close\"></i></div></div><div class=\"col-xs-6\"><h4 class=\"ac-forecast-region\">{{ region.name }} Pilot Project</h4></div><div ng-if=\"hotZone !== &quot;default&quot;\" class=\"col-xs-6 section-label full-page-link\"><div ng-click=\"viewFullPage(hotZone.subid)\" style=\"padding-top: 10px; cursor: pointer;\" class=\"pull-right\"><i class=\"fa fa-external-link\"></i><span>&nbsp; FULL PAGE</span></div></div><div class=\"row ac-forecast-dates\"><div class=\"col-md-6\"><dl><dd class=\"small\"><strong class=\"ac-text-primary\">DATE ISSUED</strong></dd><dt class=\"small\"><span class=\"ac-text-default\">{{ formatDate(hotZone.report.dateissued) || \'N/A\'| date:\'EEEE MMMM d, y h:mm a\'  | uppercase }}</span></dt></dl></div><div class=\"col-md-6\"><dl><dd class=\"small\"><strong class=\"ac-text-primary\">VALID UNTIL</strong></dd><dt class=\"small\"><span class=\"ac-text-default\">{{ formatDate(hotZone.report.datevalid)  || \'N/A\'| date:\'EEEE MMMM d, y h:mm a\' | uppercase }}</span></dt></dl></div></div><div ng-if=\"hotZone !== &quot;default&quot;\" class=\"row\"><div class=\"col-xs-12\"><h3 class=\"ac-hot-zone-headline\">{{ hotZone.report.headline }}</h3></div></div><div ng-if=\"hotZone === &quot;default&quot;\" class=\"row\"><div class=\"col-xs-12\"><h3 class=\"ac-hot-zone-headline\">A Hot Zone Report is not <br> available at this time.</h3></div></div><div ng-if=\"hotZone.report.uploads.length &gt; 0\" class=\"row\"><div class=\"col-sm-12 section-label\"><i class=\"fa fa-camera\"></i><span>   Uploads (click/tap to enlarge)</span></div><div class=\"col-sm-12\"><ul class=\"list-inline\"><li ng-repeat=\"url in hotZone.report.thumbs\"><a ng-href=\"{{url}}\" target=\"_blank\"><img ng-src=\"{{url}}\" width=\"75\" alt=\"{{hotZone.report.headline}}\"/></a></li></ul></div></div><div class=\"row\"><div class=\"col-xs-12\"><div id=\"infoAccordion\" class=\"panel-group\"><div ng-if=\"hotZone !== &quot;default&quot;\" class=\"panel panel-primary\"><div class=\"panel-heading\"><h4 class=\"panel-title\"><a href=\"\" data-target=\"#criticalSummary\" data-toggle=\"collapse\" data-parent=\"#infoAccordion\">Critical Factors Summary<i class=\"fa fa-fw fa-level-down fa-lg pull-right\"></i><small class=\"pull-right\">click to expand</small></a></h4></div><div id=\"criticalSummary\" class=\"panel-collapse collapse\"><div class=\"panel-body\"><div class=\"row\"><div class=\"col-xs-12\">Critical factors influence avalanche hazard. The more critical factors, the greater the potential for avalanches.</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Persistent avalanche problem:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.criticalFactors.persistentAvalancheProblem || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Slab avalanches in the last 48 hours:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.criticalFactors.slabAvalanches || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Signs of instability:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.criticalFactors.instability || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\"> Recent snowfall > 30cm:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.criticalFactors.recentSnowfall || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\"> Recent rainfall:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.criticalFactors.recentRainfall || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\"> Recent windloading:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.criticalFactors.recentWindloading || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\"> Significant warming:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.criticalFactors.significantWarming || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Comments:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.criticalFactors.criticalFactorsComments || \'N/A\' }}</div></div></div></div></div><div ng-if=\"hotZone !== &quot;default&quot;\" class=\"panel panel-primary\"><div class=\"panel-heading\"><h4 class=\"panel-title\"><a href=\"\" data-target=\"#terrainSummary\" data-toggle=\"collapse\" data-parent=\"#infoAccordion\">Terrain Recommendations<i class=\"fa fa-fw fa-level-down fa-lg pull-right\"></i><small class=\"pull-right\">click to expand</small></a></h4></div><div id=\"terrainSummary\" class=\"panel-collapse collapse\"><div class=\"panel-body\"><div class=\"row\"><div class=\"col-xs-12\">Based on the critical factors, AVOIDING the following is recommended to reduce risk.</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Aspect:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.terrainAvoidanceList.aspect || \'N/A\' | acFormatList }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Elevation band:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.terrainAvoidanceList.elevationBand || \'N/A\' | acFormatList }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Elevation below:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.terrainAvoidanceList.elevationBelow || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Elevation above:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.terrainAvoidanceList.elevationAbove || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Elevation between:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.terrainAvoidanceList.elevationBetweenLow || \'N/A\' }} and {{ hotZone.report.data.terrainAvoidanceList.elevationBetweenHigh || \'N/A\' }} metres</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Terrain features:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.terrainAvoidanceList.terrainFeatures || \'N/A\' | acFormatList }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Slopes steeper than:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.terrainAvoidanceList.steeperThan || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Other terrain features:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.terrainAvoidanceList.otherTerrainFeatures || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Comments:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ hotZone.report.data.terrainAvoidanceList.terrainAvoidanceComments || \'N/A\' }}</div></div></div></div></div><div ng-if=\"hotZone !== &quot;default&quot;\" class=\"panel panel-primary\"><div class=\"panel-heading\"><h4 class=\"panel-title\"><a href=\"\" data-target=\"#travelSummary\" data-toggle=\"collapse\" data-parent=\"#infoAccordion\">Travel Advice<i class=\"fa fa-fw fa-level-down fa-lg pull-right\"></i><small class=\"pull-right\">click to expand</small></a></h4></div><div id=\"travelSummary\" class=\"panel-collapse collapse\"><div class=\"panel-body\"><p>{{ hotZone.report.data.travelAdvice.travelAdviceStatement1 }}</p><p>{{ hotZone.report.data.travelAdvice.travelAdviceStatement2 }}</p><p>{{ hotZone.report.data.travelAdvice.travelAdviceStatement3 }}</p><p>{{ hotZone.report.data.travelAdvice.travelAdviceComments }}</p></div></div></div><div ng-if=\"hotZone === &quot;default&quot;\" class=\"panel panel-primary\"><div class=\"panel-heading\"><h4 class=\"panel-title\"><a href=\"\" data-target=\"#noDataSummary\" data-toggle=\"collapse\" data-parent=\"#infoAccordion\">More Information<i class=\"fa fa-fw fa-level-down fa-lg pull-right\"></i><small class=\"pull-right\">click to expand</small></a></h4></div><div id=\"noDataSummary\" class=\"panel-collapse collapse\"><div ng-bind-html=\"noDataSummary\" class=\"panel-body\"></div></div></div><div style=\"margin-bottom: 10px;\" class=\"panel panel-primary\"><div class=\"panel-heading\"><h4 class=\"panel-title\"><a href=\"\" data-target=\"#infoSummary\" data-toggle=\"collapse\" data-parent=\"#infoAccordion\">About<i class=\"fa fa-fw fa-level-down fa-lg pull-right\"></i><small class=\"pull-right\">click to expand</small></a></h4></div><div id=\"infoSummary\" class=\"panel-collapse collapse\"><div ng-bind-html=\"infoSummary\" class=\"panel-body\"></div></div></div></div></div></div></div></div>");
$templateCache.put("hot-zone-report-form.html","<div class=\"min-form\"><form name=\"acHotZoneForm\" novalidate=\"novalidate\" ng-if=\"!report.subid\" role=\"form\" ac-submission-form-validator=\"ac-submission-form-validator\"><div class=\"form-fields col-xs-12 no-padding\"><div class=\"col-xs-12 col-md-4 form-left-column\"><div class=\"required-info clearfix\"><h3 class=\"form-subtitle\">Step 1. Required Info</h3><div class=\"required-info-data col-xs-12\"><div ng-class=\"{\'has-error\': !acHotZoneForm.title.$valid &amp;&amp; acHotZoneForm.title.$dirty }\" class=\"form-group\"><h4>Headline</h4><input type=\"text\" name=\"title\" ng-model=\"report.headline\" placeholder=\"e.g. two - three sentences\" required=\"required\" class=\"form-control\"/></div><div ng-class=\"{\'has-error\': !acHotZoneForm.dateissued.$valid}\" class=\"form-group\"><h4> Date of issue</h4><input type=\"datetime\" name=\"dateissued\" ng-model=\"report.dateissued\" ac-datetime-picker=\"ac-datetime-picker\" max-date-today=\"true\" class=\"form-control\"/></div><div ng-class=\"{\'has-error\': !acHotZoneForm.datevalid.$valid}\" class=\"form-group\"><h4> Valid until</h4><input type=\"datetime\" name=\"datevalid\" ng-model=\"report.datevalid\" ac-datetime-picker=\"ac-datetime-picker\" max-date-today=\"false\" class=\"form-control\"/></div><div ng-class=\"{\'has-error\': invalidLocation}\" class=\"form-group\"><h4> Region</h4><div ng-repeat=\"option in hotZones\" style=\"margin-top: 10px;\" class=\"radio col-md-6 col-xs-12\"><label style=\"color: black;\"><input type=\"radio\" ng-model=\"report.hotzoneid\" value=\"{{option.properties.id}}\" ng-click=\"invalidLocation = false\"/>{{option.properties.name}}</label></div></div></div></div><div class=\"uploads\"><h3 class=\"form-subtitle\">Step 2. Uploads</h3><p>Add a photo (.jpg or .png).</p><div class=\"help-text\"><p><i class=\"fa fa-question-circle\"></i>If uploading more than one photo, select all and submit photos together.</p></div><div class=\"form-group\"><div class=\"col-xs-12 no-padding btn-file\"><div class=\"col-xs-3 no-padding\"><span class=\"btn btn-default btn-styled\">Browse</span></div><input type=\"text\" readonly=\"readonly\" placeholder=\".jpg or .png\" value=\"{{ report.files.length ? report.files.length + \' photos added\' : null}}\" class=\"col-xs-9 no-padding\"/><input type=\"file\" name=\"uploads\" file-model=\"report.files\" accept=\".png,.jpg,.jpeg\" multiple=\"multiple\"/></div></div></div></div><div class=\"col-xs-12 col-md-8 form-right-column\"><div class=\"row\"><div class=\"col-xs-12\"><h3 style=\"margin: 7.5px 0;\" id=\"top\" class=\"form-subtitle\">Step 3. Report Details</h3></div></div><div class=\"row\"><div class=\"col-xs-12\"><h4 class=\"form-subsection\">Critical Factors Summary</h4></div></div><div style=\"margin: 0;\" class=\"fields-group col-xs-12 no-padding\"><div ng-repeat=\"(item, av) in report.data.criticalFactors.fields\" ng-if=\"av.type!==\'calculated\'\" class=\"field col-xs-12\"><h4 ng-if=\"av.title === undefined &amp;&amp; av.subTitle === undefined\" class=\"field-title col-xs-12 no-padding\"><strong>{{ ::av.prompt }}</strong></h4><h4 ng-if=\"av.title !== undefined &amp;&amp; av.title !== null\" class=\"field-title col-xs-12 no-padding\"><strong>{{ ::av.title }}</strong></h4><h5 ng-if=\"av.subTitle !== undefined\" class=\"col-xs-12 no-padding\">{{ ::av.subTitle }}</h5><div class=\"options col-xs-12 col-md-8\"><div ng-if=\"av.helpText\" class=\"help-text\"><p><i class=\"fa fa-question-circle\"></i><span>{{ ::av.helpText}}</span><span ng-if=\"av.guidelines\"><a href=\"{{ ::av.guidelines}}\" target=\"_blank\">Submission guidelines</a></span></p></div><div ng-if=\"av.type==\'checkbox\'\" ng-repeat=\"(option, enabled) in av.options\" ng-class=\"{\'inline\':av.inline, \'column-left\':$odd}\" class=\"checkbox col-md-6 col-xs-12\"><label><input type=\"checkbox\" name=\"{{::item}}\" ng-model=\"av.options[option]\" ng-click=\"validate(item, av)\"/>{{ ::option}}</label></div><div ng-if=\"av.type==\'radio\'\" ng-repeat=\"option in av.options\" ng-class=\"{\'inline\':av.inline}\" style=\"margin: 0;\" class=\"radio col-md-12 col-xs-12\"><label><input type=\"radio\" ng-model=\"report.data.criticalFactors.fields[item].value\" value=\"{{ ::option}}\"/>{{ ::option}}</label></div><div ng-if=\"av.type==\'number\'\" ng-class=\"{\'inline\':av.inline}\" class=\"number col-xs-12\"><label><input type=\"number\" name=\"{{::item}}\" placeholder=\"{{ av.placeholder ? av.placeholder : \'Number between \'+ av.options.min +\' and \'+av.options.max }}\" ng-model=\"report.data.criticalFactors.fields[item].value\" ng-blur=\"validate(item, av)\" class=\"form-control\"/></label></div><div ng-if=\"av.type==\'text\'\" ng-class=\"{\'inline\':av.inline}\" class=\"text col-xs-12\"><label><input type=\"text\" name=\"{{::item}}\" placeholder=\"{{ ::av.prompt }}\" ng-model=\"report.data.criticalFactors.fields[item].value\" ng-required=\"report.data.criticalFactors.fields[av.constraint.field].options[av.constraint.option]\" ng-blur=\"validate(item, av)\" class=\"form-control\"/></label></div><div ng-if=\"av.type==\'dropdown\'\" ng-class=\"{\'inline\':av.inline, \'column-left\':$odd}\" class=\"select col-md-6 col-xs-12 no-padding\"><select ng-options=\"option for option in av.options\" ng-model=\"report.data.criticalFactors.fields[item].value\" class=\"form-control\"><option value=\"\">Select option</option></select></div><div ng-if=\"av.type==\'textarea\'\" class=\"textarea col-xs-12\"><textarea rows=\"3\" ng-model=\"report.data.criticalFactors.fields[item].value\" class=\"form-control\"></textarea></div><div ng-if=\"av.type==\'datetime\'\" class=\"datetime-input col-xs-12\"><label for=\"datetime\"><input type=\"datetime\" ng-model=\"report.data.criticalFactors.fields[item].value\" ac-datetime-picker=\"ac-datetime-picker\" show-only-date=\"av.showOnlyDate\" show-only-time=\"av.showOnlyTime\" max-date-today=\"av.maxDateToday\" placeholder=\"{{av.placeholder ? av.placeholder : \'Click to select date\'}}\" class=\"form-control\"/></label></div><div ng-if=\"acHotZoneForm[item].$invalid\" class=\"error col-xs-12 no-padding\">{{::av.errorMessage}}</div></div><div class=\"options col-xs-12 col-md-4\"></div></div></div><div class=\"row\"><div class=\"col-xs-12\"><h4 class=\"form-subsection\">Terrain Avoidance List</h4></div></div><div style=\"margin: 0;\" class=\"fields-group col-xs-12 no-padding\"><div ng-repeat=\"(item, av) in report.data.terrainAvoidanceList.fields\" ng-if=\"av.type!==\'calculated\'\" class=\"field col-xs-12\"><h4 ng-if=\"av.title === undefined &amp;&amp; av.subTitle === undefined\" class=\"field-title col-xs-12 no-padding\"><strong>{{ ::av.prompt }}</strong></h4><h4 ng-if=\"av.title !== undefined &amp;&amp; av.title !== null\" class=\"field-title col-xs-12 no-padding\"><strong>{{ ::av.title }}</strong></h4><h5 ng-if=\"av.subTitle !== undefined\" class=\"col-xs-12 no-padding\">{{ ::av.subTitle }}</h5><div class=\"options col-xs-12 col-md-8\"><div ng-if=\"av.helpText\" class=\"help-text\"><p><i class=\"fa fa-question-circle\"></i><span>{{ ::av.helpText}}</span><span ng-if=\"av.guidelines\"><a href=\"{{ ::av.guidelines}}\" target=\"_blank\">Submission guidelines</a></span></p></div><div ng-if=\"av.type==\'checkbox\'\" ng-repeat=\"(option, enabled) in av.options\" ng-class=\"{\'inline\':av.inline, \'column-left\':$odd, \'col-md-12\': av.prompt===\'Elevation band:\'}\" ng-style=\"av.prompt===\'Elevation band:\' &amp;&amp; {\'margin\': \'0\'}\" class=\"checkbox col-md-6 col-xs-12\"><label><input type=\"checkbox\" name=\"{{::item}}\" ng-model=\"av.options[option]\" ng-click=\"validate(item, av)\"/>{{ ::option}}</label></div><div ng-if=\"av.type==\'radio\'\" ng-repeat=\"option in av.options\" ng-class=\"{\'inline\':av.inline}\" style=\"margin-top: 0;\" class=\"radio col-md-12 col-xs-12\"><label><input type=\"radio\" ng-model=\"report.data.terrainAvoidanceList.fields[item].value\" value=\"{{ ::option}}\"/>{{ ::option}}</label></div><div ng-if=\"av.type==\'number\'\" ng-class=\"{\'inline\':av.inline}\" class=\"number col-xs-12\"><label><input type=\"number\" name=\"{{::item}}\" placeholder=\"{{ av.placeholder ? av.placeholder : \'Number between \'+ av.options.min +\' and \'+av.options.max }}\" ng-model=\"report.data.terrainAvoidanceList.fields[item].value\" ng-blur=\"validate(item, av)\" class=\"form-control\"/></label></div><div ng-if=\"av.type==\'text\'\" ng-class=\"{\'inline\':av.inline}\" class=\"text col-xs-12\"><label><input type=\"text\" name=\"{{::item}}\" placeholder=\"{{ ::av.prompt }}\" ng-model=\"report.data.terrainAvoidanceList.fields[item].value\" ng-required=\"report.data.terrainAvoidanceList.fields[av.constraint.field].options[av.constraint.option]\" ng-blur=\"validate(item, av)\" class=\"form-control\"/></label></div><div ng-if=\"av.type==\'dropdown\'\" ng-class=\"{\'inline\':av.inline, \'column-left\':$odd}\" class=\"select col-md-6 col-xs-12 no-padding\"><select ng-options=\"option for option in av.options\" ng-model=\"report.data.terrainAvoidanceList.fields[item].value\" class=\"form-control\"><option value=\"\">Select option</option></select></div><div ng-if=\"av.type==\'textarea\'\" class=\"textarea col-xs-12\"><textarea rows=\"3\" ng-model=\"report.data.terrainAvoidanceList.fields[item].value\" class=\"form-control\"></textarea></div><div ng-if=\"av.type==\'datetime\'\" class=\"datetime-input col-xs-12\"><label for=\"datetime\"><input type=\"datetime\" ng-model=\"report.data.terrainAvoidanceList.fields[item].value\" ac-datetime-picker=\"ac-datetime-picker\" show-only-date=\"av.showOnlyDate\" show-only-time=\"av.showOnlyTime\" max-date-today=\"av.maxDateToday\" placeholder=\"{{av.placeholder ? av.placeholder : \'Click to select date\'}}\" class=\"form-control\"/></label></div><div ng-if=\"acHotZoneForm[item].$invalid\" class=\"error col-xs-12 no-padding\">{{::av.errorMessage}}</div></div><div class=\"options col-xs-12 col-md-4\"></div></div></div><div class=\"row\"><div class=\"col-xs-12\"><h4 class=\"form-subsection\">Travel Advice</h4></div></div><div style=\"margin: 0;\" class=\"fields-group col-xs-12 no-padding\"><div ng-repeat=\"(item, av) in report.data.travelAdvice.fields\" ng-if=\"av.type!==\'calculated\'\" class=\"field col-xs-12\"><h4 ng-if=\"av.title === undefined &amp;&amp; av.subTitle === undefined\" class=\"field-title col-xs-12 no-padding\"><strong>{{ ::av.prompt }}</strong></h4><h4 ng-if=\"av.title !== undefined &amp;&amp; av.title !== null\" class=\"field-title col-xs-12 no-padding\"><strong>{{ ::av.title }}</strong></h4><h5 ng-if=\"av.subTitle !== undefined\" class=\"col-xs-12 no-padding\">{{ ::av.subTitle }}</h5><div class=\"options col-xs-12 col-md-8\"><div ng-if=\"av.helpText\" class=\"help-text\"><p><i class=\"fa fa-question-circle\"></i><span>{{ ::av.helpText}}</span><span ng-if=\"av.guidelines\"><a href=\"{{ ::av.guidelines}}\" target=\"_blank\">Submission guidelines</a></span></p></div><div ng-if=\"av.type==\'checkbox\'\" ng-repeat=\"(option, enabled) in av.options\" ng-class=\"{\'inline\':av.inline, \'column-left\':$odd}\" class=\"checkbox col-md-12 col-xs-12\"><label><input type=\"checkbox\" name=\"{{::item}}\" ng-model=\"av.options[option]\" ng-click=\"validate(item, av)\"/>{{ ::option}}</label></div><div ng-if=\"av.type==\'radio\'\" ng-repeat=\"option in av.options\" ng-class=\"{\'inline\':av.inline}\" style=\"margin: 0\" class=\"radio col-md-6 col-xs-12\"><label><input type=\"radio\" ng-model=\"report.data.travelAdvice.fields[item].value\" value=\"{{ ::option}}\"/>{{ ::option}}</label></div><div ng-if=\"av.type==\'number\'\" ng-class=\"{\'inline\':av.inline}\" class=\"number col-xs-12\"><label><input type=\"number\" name=\"{{::item}}\" placeholder=\"{{ av.placeholder ? av.placeholder : \'Number between \'+ av.options.min +\' and \'+av.options.max }}\" ng-model=\"report.data.travelAdvice.fields[item].value\" ng-blur=\"validate(item, av)\" class=\"form-control\"/></label></div><div ng-if=\"av.type==\'text\'\" ng-class=\"{\'inline\':av.inline}\" class=\"text col-xs-12\"><label><input type=\"text\" name=\"{{::item}}\" placeholder=\"{{ ::av.prompt }}\" ng-model=\"report.data.travelAdvice.fields[item].value\" ng-required=\"report.data.travelAdvice.fields[av.constraint.field].options[av.constraint.option]\" ng-blur=\"validate(item, av)\" class=\"form-control\"/></label></div><div ng-if=\"av.type==\'dropdown\'\" ng-class=\"{\'inline\':av.inline, \'column-left\':$odd}\" class=\"select col-md-6 col-xs-12 no-padding\"><select ng-options=\"option for option in av.options\" ng-model=\"report.data.travelAdvice.fields[item].value\" class=\"form-control\"><option value=\"\">Select option</option></select></div><div ng-if=\"av.type==\'textarea\'\" class=\"textarea col-xs-12\"><textarea rows=\"3\" ng-model=\"report.data.travelAdvice.fields[item].value\" class=\"form-control\"></textarea></div><div ng-if=\"av.type==\'datetime\'\" class=\"datetime-input col-xs-12\"><label for=\"datetime\"><input type=\"datetime\" ng-model=\"report.data.travelAdvice.fields[item].value\" ac-datetime-picker=\"ac-datetime-picker\" show-only-date=\"av.showOnlyDate\" show-only-time=\"av.showOnlyTime\" max-date-today=\"av.maxDateToday\" placeholder=\"{{av.placeholder ? av.placeholder : \'Click to select date\'}}\" class=\"form-control\"/></label></div><div ng-if=\"acHotZoneForm[item].$invalid\" class=\"error col-xs-12 no-padding\">{{::av.errorMessage}}</div></div><div class=\"options col-xs-12 col-md-4\"></div></div></div></div></div><div class=\"footer-buttons col-xs-12 no-padding\"><input type=\"button\" value=\"Back\" ng-click=\"goBack(acHotZoneForm.$dirty)\" class=\"btn btn-default btn-styled\"/><div class=\"submit-btn\"><i ng-show=\"submitting\" class=\"fa fa-fw fa-lg fa-spinner fa-spin\"></i><input type=\"button\" ng-click=\"submitForm()\" id=\"submit\" value=\"SUBMIT\" ng-disabled=\"report.latlng.length === 0 || submitting\" class=\"btn btn-default btn-styled\"/></div></div></form><div ng-if=\"error\"><div role=\"alert\" class=\"alert alert-danger\"><p>There was an error submitting your report.</p><p ng-if=\"errormsg\">{{errormsg}}</p></div></div></div>");
$templateCache.put("hot-zone-report-fullpage.html","<div class=\"panel\"><div class=\"panel-body\"><div class=\"row\"><header class=\"col-xs-12\"><div id=\"header-inner\"><h1>{{ report.hotzoneid | acCapitalize }} Pilot Project</h1><div ng-click=\"print()\" class=\"section-label print-link pull-right\"><i class=\"fa fa-external-link\"></i><span>&nbsp; Print Friendly</span></div></div></header><section id=\"forecast-date\" class=\"col-xs-12\"><div id=\"date-issued\" class=\"forecast-date-inner\">Date Issued &nbsp;<br class=\"visible-xs-inline\"/><span> {{ formatDate(report.report.dateissued) | date:\'EEE MMMM d, h:mm a\' | uppercase }}</span></div><div id=\"date-valid\" style=\"width: 574px;\" class=\"forecast-date-inner\">&nbsp; Valid Until &nbsp;<br class=\"visible-xs-inline\"/><span>{{ formatDate(report.report.datevalid) | date:\'EEE MMMM d, h:mm a\' | uppercase }}</span></div><div id=\"date-issued\" ng-if=\"report.user\" class=\"forecast-date-inner\">Forecaster &nbsp;<br class=\"visible-xs-inline\"/><span>{{ report.user }}</span></div><div id=\"date-valid\" ng-if=\"report.user\" style=\"width: 574px;\" class=\"forecast-date-inner\"></div></section><section id=\"forecast-body\" class=\"col-xs-12 col-md-12\"><div id=\"highlights\"><h2>{{report.report.headline}}</h2></div><div ng-if=\"report.report.uploads.length &gt; 0\" class=\"upload-n-share\"><div class=\"row-fluid observation-title\">Uploads</div><div class=\"row\"><div class=\"col-sm-12 section-label\"><i class=\"fa fa-camera\"></i><span>Uploads (click/tap to enlarge)</span></div><div class=\"col-sm-12\"><ul class=\"list-inline\"><li ng-repeat=\"url in report.report.thumbs\"><a ng-href=\"{{::url}}\" target=\"_blank\"></a><img ng-src=\"{{::url}}\" width=\"75\" alt=\"{{::report.report.headline}}\"/></li></ul></div></div></div><div class=\"panel-group\"><div class=\"panel panel-primary\"><div class=\"panel-heading\">Critical Factors Summary</div><div class=\"panel-body\"><div class=\"row\"><div class=\"col-xs-12\">Critical factors influence avalanche hazard. The more critical factors, the greater the potential for avalanches.</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Persistent avalanche problem:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.criticalFactors.persistentAvalancheProblem || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Slab avalanches in the last 48 hours:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.criticalFactors.slabAvalanches || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Signs of instability:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.criticalFactors.instability || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\"> Recent snowfall > 30cm:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.criticalFactors.recentSnowfall || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\"> Recent rainfall:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.criticalFactors.recentRainfall || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\"> Recent windloading:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.criticalFactors.recentWindloading || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\"> Significant warming:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.criticalFactors.significantWarming || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Comments:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.criticalFactors.criticalFactorsComments || \'N/A\' }}</div></div></div></div><div class=\"panel panel-primary\"><div class=\"panel-heading\">Terrain Recommendations</div><div class=\"panel-body\"><div class=\"row\"><div class=\"col-xs-12\">Based on the critical factors, AVOIDING the following is recommended to reduce risk.</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Aspect:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.terrainAvoidanceList.aspect || \'N/A\' | acFormatList }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Elevation band:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.terrainAvoidanceList.elevationBand || \'N/A\' | acFormatList }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Elevation below:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.terrainAvoidanceList.elevationBelow || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Elevation above:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.terrainAvoidanceList.elevationAbove || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Elevation between:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.terrainAvoidanceList.elevationBetweenLow || \'N/A\' }} and {{ report.report.data.terrainAvoidanceList.elevationBetweenHigh || \'N/A\' }} metres</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Terrain features:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.terrainAvoidanceList.terrainFeatures || \'N/A\' | acFormatList }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Slopes steeper than:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.terrainAvoidanceList.steeperThan || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Other terrain features:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.terrainAvoidanceList.otherTerrainFeatures || \'N/A\' }}</div></div><div class=\"row\"><div class=\"col-xs-6 sm-pad-right\"><strong class=\"pull-right text-right\">Comments:</strong></div><div class=\"col-xs-6 sm-pad-left\">{{ report.report.data.terrainAvoidanceList.terrainAvoidanceComments || \'N/A\' }}</div></div></div></div><div class=\"panel panel-primary\"><div class=\"panel-heading\">Travel Advice</div><div class=\"panel-body\"><p>{{ report.report.data.travelAdvice.travelAdviceStatement1 }}</p><p>{{ report.report.data.travelAdvice.travelAdviceStatement2 }}</p><p>{{ report.report.data.travelAdvice.travelAdviceStatement3 }}</p><p>{{ report.report.data.travelAdvice.travelAdviceComments }}</p></div></div><div class=\"panel panel-primary\"><div class=\"panel-heading\">More Information</div><div ng-bind-html=\"infoSummary\" class=\"panel-body\"></div></div></div></section></div></div></div>");
...
$templateCache.put("spring-conditions.html","<div class=\"col-xs-12\"><div class=\"panel panel-primary\"><div class=\"panel-heading\">Spring Conditions: {{ forecast.dangerRatings[0].date | dateUtc:\'dddd\' }}</div><div class=\"panel-body\"><div class=\"col-xs-4\"><img style=\"max-width:90%\" src=\"http://www.avalanche.ca/assets/images/spring_situation_icon.svg\"/></div><div class=\"col-xs-8\"><ac-spring-conditions-text/></div></div></div></div>");
